...
public class StatusBarHelper {
    ...
    private static void MIUISetStatusBarLightMode(Object object, boolean dark) {
        ...
        if (window != null) {
            ...
            try {
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M && RomHelper.isMiUIV7OrAbove()) {
    ...
    public static void setLightStatusBar(final Activity activity, final boolean dark, boolean isFullMode) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            int DEVICE_FIRM = -1;
            if (DEVICE_FIRM == -1) {
                DEVICE_FIRM = RomHelper.getLightStatusBarAvailableRomType();
            }
            switch (DEVICE_FIRM) {
                case RomHelper.AvailableRomType.MIUI:
                    MIUISetStatusBarLightMode(activity, dark);
                    break;

                case RomHelper.AvailableRomType.FLYME:
                ...
                case RomHelper.AvailableRomType.ANDROID_NATIVE:
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                        setAndroidNativeLightStatusBar(activity, dark, isFullMode);
                    }
                    break;

                case RomHelper.AvailableRomType.NA:
                    // N/A do nothing
                    break;
            }
        }
    ...
    public static void setLightStatusBar(final Window window, final boolean dark, boolean isFullMode) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            switch (RomHelper.getLightStatusBarAvailableRomType()) {
                case RomHelper.AvailableRomType.MIUI:
